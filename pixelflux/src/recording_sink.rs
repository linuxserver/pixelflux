...
impl RecordingSink {
    /// Empty path = recording off (returns None).
    ...
    /// Fans encoded bytes out to every client without blocking the caller: bytes go
    /// to each client's bounded queue; a full (too slow) or disconnected client is
    /// dropped. `data` is a raw Annex-B H.264 byte slice.
